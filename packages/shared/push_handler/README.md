# Shared Push Handler (Laravel Package)
...
## Package Location
`laravel-app/packages/shared/push_handler`
...
2. Register the provider (already registered in this repo):
   - `Shared\PushHandler\PushHandlerServiceProvider`
3. Ensure host config is present:
   - `laravel-app/config/shared_push_handler.php`

## Configuration

### Route Paths (Host Controlled)
Host app owns route path strings. Package reads them from:
- `config('shared_push_handler.routes')`
...
```php
Shared\PushHandler\Contracts\PushPlanPolicyContract
```
...
The package registers routes but reads paths from `config('shared_push_handler.routes')`. Override the host config file to change them.
